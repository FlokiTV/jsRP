console.log("[jsrp:emote] starting");
...
RegisterKeyMapping("+clear_emote", "Limpar emotes", "keyboard", "f6");

RegisterCommand("e", (source, args) => {
  let player = GetPlayerPed(-1);
  let [emote] = args;
  let em = emotes.filter((e) => e.nome == emote);
  ClearPedTasks(player);
  PLAYER.status = false;
  console.log("emote " + em[0]);
  if (!PLAYER.status && em.length) {
    RequestAnimDict(em[0].dict);
    PLAYER.status = true;
    let i = setInterval(() => {
      if (HasAnimDictLoaded(em[0].dict)) {
        TaskPlayAnim(player, em[0].dict, em[0].anim, 4, 4, -1, 1, 0);
        clearInterval(i);
      }
    }, 500);
  }
